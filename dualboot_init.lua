...
if not cp.invoke(computer.getBootAddress(), "exists", "/lib/core/boot.lua") then -- OpenOS doesn't exists, so always boot EngyOS
	_G.loadfile = loadfile
	loadfile("EngyOS/Kernel/boot.lua")()
	return
elseif not cp.invoke(computer.getBootAddress(), "exists", "/EngyOS/Kernel/boot.lua") then -- EngyOS doesn't exists, so always boot OpenOS
...
gpu.fill(1, 1, 22, 2, ' ')
gpu.set(1, 1, "Press ENTER for EngyOS")
